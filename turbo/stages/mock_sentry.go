...
func MockWithEverything(t *testing.T, gspec *core.Genesis, key *ecdsa.PrivateKey, prune prune.Mode, engine consensus.Engine, withTxPool bool, withPosDownloader bool) *MockSentry {
	...
	mock.Sync = stagedsync.New(
		stagedsync.DefaultStages(mock.Ctx, nil, prune,
			...
			stagedsync.StageFinishCfg(mock.DB, dirs.Tmp, nil, nil, nil),
			!withPosDownloader),
	...
	mock.MiningSync = stagedsync.New(
		stagedsync.MiningStages(mock.Ctx,
			...
			stagedsync.StageMiningExecCfg(mock.DB, miner, nil, *mock.ChainConfig, mock.Engine, &vm.Config{}, dirs.Tmp, nil, nil),
...
func (ms *MockSentry) insertPoWBlocks(chain *core.ChainPack) error {
	...
	if _, err = StageLoopStep(ms.Ctx, ms.DB, ms.Sync, highestSeenHeader, ms.Notifications, initialCycle, ms.UpdateHead, nil, nil); err != nil {
		return err
	}
	if ms.TxPool != nil {
...
func (ms *MockSentry) insertPoSBlocks(chain *core.ChainPack) error {
	...
	headBlockHash, err := StageLoopStep(ms.Ctx, ms.DB, ms.Sync, highestSeenHeader, ms.Notifications, initialCycle, ms.UpdateHead, nil, nil)
	if err != nil {
		return err
	}
